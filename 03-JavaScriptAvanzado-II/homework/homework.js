...
function cacheFunction(cb) {
  ...
  Tu tarea aquí es lograr, mediante un closure, que cacheFunction actúe como una memoria caché para el callback que recibe por parámetro (cb);
   es decir, que "recuerde" el resultado de cada operación que hace, de manera que,
    al realizar una operación por segunda vez, se pueda obtener el resultado de 
    esa "memoria" sin tener que efectuar otra vez cálculos que ya se hicieron anteriormente.

  cacheFunction debe retornar una función. Esta función debe aceptar un argumento (arg)
  e invocar a cb con ese argumento; hecho eso, debe guardar el argumento junto con el resultado
  de la invocación (tip: usá un objeto donde cada propiedad sea el argumento,
  ...
  */
  ...
  //creamo el objeto que servira de cache
  var cache = new Object();


   return function(arg){
      //comprobamos si el argumento recibido ya esta
      if(cache.hasOwnProperty(arg)){
        
        //en caso de estar, lo returnamos de la cache
       return cache[arg];

      }else{

        //si es primera vez que se ejecuta el arg, lo guardamos y lo retornamos
         cache[arg] = cb(arg);
         return cache[arg];
      }
...
/*
  ...
  tres funciones que retornen una cadena (string) y el delimitador especificado (asteriscos, guiones,
    y guiones bajos, respectivamente). Las funciones obtenidas deberían recibir 
...
function asterisco(str){
  return this("*", "*", str); 
}
function guion(str){
  return this("-", "-", str);
}
function guionBajo(str){
  return this("_", "_", str);
}



let textoAsteriscos = asterisco.bind(crearCadena);
let textoGuiones = guion.bind(crearCadena);
let textoUnderscore = guionBajo.bind(crearCadena);
